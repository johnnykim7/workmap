...
- **CR-031 VIEWER 읽기전용 강제 — 쓰기 @PreAuthorize 가드 + BE 운영배포·E2E 완료(2026-06-29 구현·2026-07-02 배포, 소규모, 커밋 b05e397)**: 역할 질문 중 코드 실측으로 VIEWER 읽기전용(POL-004)이 코드상 미강제였음 발견. 업무 쓰기 엔드포인트에 가드가 없어 VIEWER도 생성/수정/삭제 가능했음. **WmpAuthz.WRITER**(=hasAnyRole OWNER/ADMIN/MANAGER/MEMBER, VIEWER 제외) 공용 상수 → WorkItem 10·SubResource 4·Sprint 3·Ops 2·Approval 1 쓰기 메서드에 @PreAuthorize(GET 미적용=조회 허용 / 저장필터·채팅은 예외로 VIEWER 허용). FE useCanWrite 헬퍼 + 헤더 만들기버튼 VIEWER 숨김. 설정(/admin)·프로젝트·WS·초대는 기존 가드로 이미 VIEWER 차단됨. **BE 212/212 PASS**. **운영 배포·E2E 완료(2026-07-02)**: deploy.sh be — VIEWER 계정으로 업무생성 403(code 7005)·상태변경 403·조회 200 확인. (⚠️ 첫 확인 시 CR-031 가드가 이전 배포 jar에 없어 400=DTO검증까지 통과됨 발견 → 재배포 후 403 정상. jar unzip grep NO_WmpAuthz는 상수 인라인이라 검사 한계일 뿐 가드는 403으로 실동작.) 테스트 VIEWER 계정 비활성화 정리. **남은 한계**: project_members.role(프로젝트별 역할)은 여전히 전역 역할만 강제 — 별도 후속.
- **CR-027 이메일 초대 가입 + 비밀번호 재설정/변경(인증번호 OTP) — 설계+구현+BE 운영배포·E2E 완료(2026-06-29, 대규모, 커밋 e598eaa·버그픽스 추가)**: 사용자 지시 "회원가입 시 초대 이메일→링크 진입→비밀번호 설정, 변경도 같은 메커니즘, 이메일은 bp-notification 소비앱 등록".
  ...
  - **⚠️ CR-027 보정(토큰 전환) — 초대·분실재설정 OTP→링크 토큰(2026-06-29, 설계 cb3902a·구현 후속 커밋, BE 운영배포·API E2E 완료)**: 운영 초대 메일에 **수락 링크가 없어** "인증번호만 받고 어디로 가는지 모르는" 문제(사용자 캡처) + 사용자 정리 "링크 방식이면 인증번호 불필요, 링크 아닌 것(forgot 등)에만 필요" → **메일 링크 클릭=메일함 접근=본인확인**이라 초대·forgot 둘 다 링크면 OTP 불필요. (애초 첫 요구 "링크 타고 들어오면 비번 설정"으로 원복.) **결정**: 초대(INVITE)·분실재설정(RESET)=토큰 링크(인증번호 제거), 변경(CHANGE)만 OTP 유지. 토큰 정책 POL-013-A(추측불가 랜덤·SHA-256 해시·초대72h/재설정30분·1회용·시도제한 불요). **BE**: V11(invitations.token_hash + password_reset_tokens, email_otp는 CHANGE 전용 축소) + `TokenService`(32B 랜덤·SHA-256) + InvitationService/PasswordService 토큰 재작성 + AuthController `GET /auth/invitations/{token}` 미리보기·accept/reset 토큰 바디 + SecurityWhitelist `/auth/invitations/*` + `workmap.notification.web-base-url`(메일 링크 절대 URL) + 에러코드 INVITATION_TOKEN_INVALID(**7841**)·RESET_TOKEN_INVALID(**7842**). **BE 213/213 PASS**. **bp-notification 템플릿 교체**: WMP_INVITE_OTP/RESET_OTP → **WMP_INVITE_LINK·WMP_RESET_LINK**(actionUrl 버튼, jq로 등록). **FE**: InviteAcceptPage(토큰 미리보기 GET+비번만)·PasswordForgotPage(이메일만)·**PasswordResetPage 신규**(`/password/reset?token=`)·api/hooks 토큰화. tsc+build·api계약 10/10. **운영 배포·API E2E 완료**: deploy.sh be(V11 Flyway success·token_hash 컬럼·password_reset_tokens 생성) — 초대 생성→**WMP_INVITE_LINK 실발송**·forgot admin→**WMP_RESET_LINK 실발송**+reset 토큰 적재·무효 토큰 미리보기/수락 7841·무효 reset 7842·공개경로 무인증 410(403 아님) 전부 통과. **화면 끝단 E2E(메일 링크 클릭→비번 설정 가입)는 FE 배포 후**(CR-028 동행). 상세 [[workmap-bp-notification]].
- **CR-032 가입 요청(셀프 신청 → 관리자 승인 → 초대 발송) — 설계+구현+BE 운영배포·API E2E 완료(2026-07-02, 중규모, 커밋 후속)**: 사용자 지시 "회원가입 창에서 반대로 사용자가 관리자에게 회원가입 요청하는 경우". 기존은 관리자 초대(push)만 → 셀프 신청(pull) 추가. **결정**: 승인 시 관리자가 역할 지정→**기존 초대(CR-027 토큰 링크) 재사용** 발송 / 입구=로그인 화면 "가입 요청" 링크 / 관리자 알림=받은함·목록 표시만(별도 발송 없음) / 토큰 재설계 완료 후 착수. **설계 캐스케이드**(T1-1 WMP-AUTH-010·T3-1 signup_requests·T3-2·T3-3·CR이력, 커밋 1572b9f). **BE**: V12(signup_requests, PENDING/APPROVED/REJECTED, 대기 부분유니크) + 에러코드 WMP-7843~7845 + SignupRequest 도메인/매퍼/DTO/Service/Controller + 신청=공개(AuthController POST /auth/signup-requests + Whitelist) + 목록·승인·거절=Admin(SignupRequestController, 승인→InvitationService.invite 위임) + @WebMvcTest 3슬라이스 MockBean. **BE 220/220 PASS**(신규 7). **FE**: SignupRequestPage(/signup-request 공개)·로그인 "가입 요청" 링크·features/invitation signup-api/hooks·SignupRequestsSection(UsersPage 삽입, 역할지정 승인·거절). tsc+build·api 14/14. **운영 배포·API E2E 완료**: deploy.sh be(V12 Flyway·signup_requests 생성) — 신청 무인증 200·중복 7845·가입이메일 7741·목록 무인증 403·**Admin 승인→초대 생성(invitations)+WMP_INVITE_LINK 발송**(signup=APPROVED·invitation=PENDING 확인)·재승인 7844 전부 통과. **⚠️ E2E 파싱 착오**: psql 멀티statement에서 invitations id를 signup_requests id로 오독→"모순" 오판했으나 재확인 결과 전부 정상. **⚠️ E2E 파싱 착오**: psql 멀티statement에서 invitations id를 signup_requests id로 오독→"모순" 오판했으나 재확인 결과 전부 정상.
- **CR-033 초대·가입승인에 워크스페이스 지정 + 수락 시 자동 합류 — 설계+구현+운영배포(BE·FE)·E2E 완료(2026-07-02, 중규모, 커밋 2c4bf78)**: 운영 확인에서 **관리자 초대로 가입 완료한 사용자가 "속한 워크스페이스가 없습니다" 빈 화면에 갇힘** 발견(사용자 캡처). 실측: InvitationService.accept에 WS 합류가 전혀 없었음(user만 생성) → CR-018 WS격리(BIZ-112)로 신규 가입자는 아무 WS도 못 봄. **설계 구멍**. **결정(사용자 합의)**: ① 초대=관리자가 초대 시 WS 지정(본인 속한 WS로 부름) ② 가입요청(CR-032)=사용자는 WS 안 고름(WS 모델상 외부인이 WS 목록 볼 수 없음·BIZ-112) → 관리자가 **승인 시 역할+WS 지정** ③ 수락 시 지정 WS 자동 합류(WS없이 초대도 허용=nullable). 슬랙 등도 셀프가입은 도메인 기반이며 외부인이 WS 고르지 않음(사용자 관찰 "WS 제품에서 사용자가 먼저 요청하는 걸 본 적 없다"와 일치). **설계 캐스케이드**(T1-1 004/006/010·T3-1 invitations.workspace_id V13·T3-2·T3-3·CR이력, 커밋 6a65e6a). **BE**: V13(workspace_id nullable) + Invitation/InviteRequest/ApproveRequest에 workspaceId + accept 시 `if(wsId!=null) workspaceService.addMember`(같은 트랜잭션, ON CONFLICT DO NOTHING) + SignupRequestService.approve가 WS 전달. INV-7 테스트. **BE 221/221 PASS**. **FE**: InviteDialog WS Select(기본=workspace-store 현재 WS·필수)·SignupRequestsSection 승인에 WS Select(미선택 시 승인 비활성). api 14/14, tsc+build. **운영 배포·E2E 완료(BE+FE 둘 다)**: V13 Flyway·workspace_id 컬럼 확인 + WS지정 초대→workspace_id 저장 + **검증토큰으로 수락→user 생성+workspace_members 자동 합류(WS=2) 확인**(빈 화면 문제 해소 실증)·초대 ACCEPTED. 검증 계정 정리. **FE도 배포 완료**(chat 빌드 깨짐 이미 해소돼 있었음 — CR-027/031/032 FE도 이번에 함께 운영 반영). 상세 [[workmap-bp-notification]].
...
- **CR-029 모바일 앱(Capacitor 래핑) — 설계+App Shell 구현 완료(2026-06-29, 중규모, 커밋 43c2577)**: App Shell(WMP-APP-001) 구현 완료 — frontend에 @capacitor/core·cli·ios·android **8.4.1**(push/camera/voice 제외) + `capacitor.config.ts`(appId=com.therecommerce.workmap·webDir=dist·allowNavigation=[59.8.160.12]·cleartext·SplashScreen) + `api-client.ts` 네이티브 분기(`Capacitor.isNativePlatform()`이면 운영BE 절대URL `http://59.8.160.12:8186/api/v1`, 웹은 기존 상대경로/MSW 유지) + package.json 스크립트(build:mobile·sync·sync:ios·sync:android·open:ios·open:android) + `npx cap add ios/android`(네이티브 프로젝트 생성, 산출물은 자체 .gitignore 제외 — 커밋엔 설정 72파일만). tsc+vite 빌드 통과, `cap sync`로 config 양 플랫폼 반영 확인. **시뮬레이터 실기동(`nvm use 22 && pnpm sync:ios && pnpm open:ios` → Xcode ▶)은 사용자 검증 몫(미수행)**. 푸시(WMP-APP-002)·카메라(003)·음성(004)은 설계만 선기재, 후속 CR. **LNB 우회 곁다리 작업**: 이 세션 중 LNB 클릭 안 됨 발견 → 근본=ds-ui AdminShell이 linkComponent 분기를 9f78282에서 제거(회귀). ds-ui 소스 복원(href+to 둘 다, react-router는 to 필요) → WorkMap react-table 설치(ds-ui dist의 external peer 충족, flexRender 빌드 깨짐 해소) + mock 계정 @workmap.com 통일(커밋 0c44ba3). **ds-ui는 design-system 레포라 담당자 커밋 몫**(patch=/tmp/ds-ui-adminshell-linkcomponent-restore.patch). 운영 FE 배포(deploy.sh fe) 완료(내 커밋만, chat·main.css 시반테마는 stash 제외 후 복구).
- **CR-029(원본) 설계 캐스케이드 메모(2026-06-29, 중규모)**: 사용자 "앱용으로도 제작, 할 수 있는 것 위주로. 추후 카메라·푸시·음성도". 셋이 로드맵 확정이라 PWA의 iOS 제약(웹푸시·STT 반쪽) 회피 위해 **Capacitor 래핑** 채택(현 React/Vite 자산 재활용, RN 안 씀). 참고=`dev-labs/bp-issues-front`(동일 조직 Capacitor 8+push, `capacitor.config.ts`·`usePushNotifications` 훅 실측). **결정(사용자 합의)**: 방식=Capacitor / 타깃=iOS·Android 둘 다(Xcode·AndroidStudio 설치 확인) / **CR-029 범위=App Shell(WMP-APP-001)만 구현**, 푸시·카메라·음성(002~004)은 **설계만 선기재**·구현은 후속 CR / API=운영 BE 절대URL 고정(`http://59.8.160.12:8186`, 앱은 dev프록시 없음) / **BE 변경 0**. **실측 확인**: BE `POST·DELETE /api/v1/fcm/token` 이미 존재(CR-028 FcmTokenController), FE `registerFcmToken`/`deleteFcmToken` 이미 존재(미배선), 파일 업로드 `/files/upload`(CR-024) 재사용 가능. **설계 캐스케이드 완료**: T1-1(모듈 J 신규·WMP-APP-001~004)·execution-spec(§CR-029 횡단 가이드)·T3-3(모바일 앱 셸 항목)·CR_변경_이력(CR-029)·CLAUDE.md. T1-3/4/5/6·T3-1/2 무변경(앱 패키징은 비즈규칙·FSM·이벤트·스키마·API 변경 아님). 에러코드·마이그레이션 없음. **App Shell 구현 작업(미착수)**: frontend에 `@capacitor/core·cli·ios·android` 의존(푸시/카메라/음성 플러그인 제외) + `capacitor.config.ts`(appId=`com.therecommerce.workmap`·webDir=`dist`·allowNavigation=[`59.8.160.12`]·cleartext·SplashScreen) + api-client baseURL 네이티브 분기(운영BE 절대URL) + package.json 스크립트(build:mobile·sync·open) + `npx cap add ios/android` + 시뮬레이터 기동 확인. ⚠️ Capacitor 의존 추가 후 [[workmap-tailwind-lnb-trap]] LNB 재확인 필수.
- **CR-037 첨부 실파일 업로드 전환 + 공통 첨부 컴포넌트·파일 뷰어 — 설계+구현+운영배포(BE·FE)·BE E2E 완료(2026-07-04, 중규모, 커밋 e174548·e7b668c·fe311da)**: 업무상세/생성 첨부가 "파일명+경로/URL 텍스트 수동입력"이라 실제 파일 첨부 불가(사용자 캡처)였고 첨부 삭제도 전 계층 부재였음. 사용자 지시로 첨부·리스트·뷰어를 **공통 재사용 컴포넌트**로 개발. **결정(사용자)**: 캡처 전체 UI(그리드/목록·…메뉴·행별 미리보기/다운로드/삭제) / 삭제=**디스크 파일도 물리삭제** / 이미지 뷰어 줌·팬=**의존성0 자체구현**([[workmap-tailwind-lnb-trap]] 회피) / PDF=iframe / 에디터 인라인 이미지 클릭도 동일 뷰어. **FE 공통 자산**: `components/common/file-attachments/`(FileAttachmentList — 어댑터 패턴 `{items,create,remove,canWrite}` 주입) + `components/common/file-viewer/`(전역 라이트박스 FileViewerProvider, main.tsx 마운트). 소비처=업무 Attachments.tsx(어댑터 래퍼)·RichTextEditor 읽기전용 img onClick. **BE**: `DELETE /work-items/{id}/attachments/{attachmentId}`(WRITER 가드) 메타+디스크 삭제(FileStorageService.deleteByPath best-effort) + AttachmentMapper findById/deleteById + application.yml 업로드 화이트리스트 이미지4종→PDF·office·zip·txt·csv 확장 + ATTACHMENT_NOT_FOUND **WMP-7846**. **⚠️CR번호**: 035=간트·036=백로그토글 점유라 첨부=**037**(착수 시 오부여 정정). **BE 224 PASS, FE tsc+build·유닛28 PASS**. **운영 배포·BE E2E 완료**: deploy.sh be+fe — PNG·PDF 업로드(화이트리스트 확장 실증)·첨부등록·무인증서빙200·삭제200·**삭제후 서빙404(디스크 물리삭제 실증)**·없는첨부 7846 / 배포 CSS에 LNB 보호규칙 잔존 확인(FileViewer fixed 오버레이 추가에도 회귀 없음). 상세 [[workmap-attachments]]. **남은 것=브라우저 화면 E2E(그리드→이미지클릭 줌뷰어→삭제)는 사용자 몫.**
- **CR-039 병렬 스프린트 + 보드 스프린트별 아코디언 (SPR-1 폐기) — 설계+구현+운영배포(BE·FE)·E2E 완료(2026-07-05, 대규모, 커밋 99f2ed9·c263a5c)**: Jira처럼 한 프로젝트에 여러 스프린트 동시 진행 + 보드를 ACTIVE 스프린트별 아코디언으로 세로 나열. 실측 확인: ① `SprintService.start`에 SPR-1 가드(동시 ACTIVE 1개, `ACTIVE_SPRINT_EXISTS`)가 병렬 차단 ② `BoardService`가 `findActiveByProject`(LIMIT 1)로 단일 스프린트만 잡아 그룹 구조 전무. 앞선 "팀 도입/owner_id 필요"는 실측으로 부정(스프린트에 팀/인원 안 붙임이 Jira 정합). **결정(사용자 2026-07-05)**: SPR-1 폐기(여러 ACTIVE 허용)·그룹기준=스프린트별(에픽 아님)·표시범위=ACTIVE 전부만(백로그/FUTURE/COMPLETED 제외)·각 섹션 독립 DnD. **설계 캐스케이드**(T1-1 WMP-AGL-005·T1-5 sprint규칙·불변식7·T3-2 /board §G·/start·T3-3 보드화면·execution-spec §CR-039·CR이력, 커밋 99f2ed9). **BE**: SprintService.start SPR-1 가드 삭제 + SprintMapper.findAllActiveByProject 신규(id오름차순) + BoardDtos = `BoardResponse{projectId,workflowId,List<SprintGroup> groups}`+`SprintGroup{sprintId,sprintName,startDate,endDate,columns}`(단수 sprintId 제거) + BoardService.board ACTIVE순회 그룹생성(columnsOf 헬퍼, 0개면 null그룹1개). **스키마 무변경·에러코드 0**(`ACTIVE_SPRINT_EXISTS` 코드 잔존·미사용). **BE 233 PASS**. **FE**: board/api.ts groups[] + hooks.ts moveCard 그룹인지형(카드 속한 그룹만 이동·섹션 오염방지) + BoardAccordionSection 신규(스프린트별 접기/펼치기·운영형은 헤더없이) + KanbanBoard columns+projectId 시그니처(그룹별 독립 DnDContext) + BoardView 단일그룹=전체화면·병렬=아코디언 세로나열 + BulkEditBar/DetailHeader groups 반영(board.columns 참조 정정). tsc+vite build 통과, board 7 PASS(병렬 오염방지 테스트 포함). **운영 배포(deploy.sh all)·E2E 완료**: 보드 응답 top keys=[projectId,workflowId,groups] 확인 + **Sprint 3 ACTIVE 중 Sprint 4 시작 허용**(SPR-1 폐기 실증, 구로직이면 거부) + 보드 groups 2개(Sprint3 cards14·Sprint4 cards11) 확인. ⚠️ 운영 RTN24(WS9)에 Sprint 3·4 병렬 ACTIVE 상태로 둠(데모용). **남은 것=브라우저 화면 E2E(보드 탭→두 스프린트 아코디언 세로 렌더·섹션별 드래그)는 사용자 몫.**
