...
> **CR-027 주의**: users 스키마는 **무변경**. 초대 단계에서는 user를 만들지 않고, 초대 수락(인증번호+비밀번호 설정) 완료 시점에만 user를 생성하므로 `password_hash NOT NULL` 불변식이 유지된다.

### invitations (사용자 초대) — WMP-AUTH-004, CR-027
| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BIGINT | PK | auto | |
...
| expires_at | TIMESTAMPTZ | Y | | 초대 만료(예: 발급+72h) |
| accepted_at | TIMESTAMPTZ | N | | 수락 완료 시각 |
| created_at | TIMESTAMPTZ | Y | now() | |
| updated_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_invitations_email`(email), `idx_invitations_status`(status). **부분 유니크**: `uniq_invitations_pending_email`(email) WHERE status='PENDING' — 같은 이메일에 PENDING 초대 중복 차단.
**규칙**: 인증번호 자체는 본 테이블이 아니라 `email_otp`에 보관(용도 INVITE). 초대 만료(`expires_at`)와 인증번호 만료(`email_otp.expires_at`)는 별개.

### email_otp (이메일 인증번호) — WMP-AUTH-006/007/008/009, POL-013, CR-027
> 초대·분실재설정·변경의 본인확인 인증번호를 단일 테이블로 통일. 인증번호 평문 미저장(해시 보관).

| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BIGINT | PK | auto | |
| email | VARCHAR(255) | Y | | 수신 이메일(로그인 못 하는 경우도 있어 user_id 대신 email 기준) |
| purpose | VARCHAR(20) | Y | | INVITE / RESET / CHANGE |
| code_hash | VARCHAR(255) | Y | | 6자리 인증번호 해시(BCrypt 또는 SHA-256+salt) |
| user_id | BIGINT | N | | → users.id (CHANGE는 로그인 사용자 id, INVITE/RESET은 null 가능) |
| attempt_count | INT | Y | 0 | 검증 시도 횟수(최대 5, POL-013) |
| consumed_at | TIMESTAMPTZ | N | | 검증 성공·소비 시각(1회용) |
| expires_at | TIMESTAMPTZ | Y | | 발급+10분(POL-013) |
| created_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_email_otp_lookup`(email, purpose, consumed_at), `idx_email_otp_expires`(expires_at).
**규칙**: 같은 (email, purpose)에 새 인증번호 발급 시 이전 미소비 인증번호를 무효화(consumed/삭제). 검증은 미소비·미만료·attempt_count<5인 최신 레코드 대상. 재발송 쿨다운 60초는 최근 created_at 비교로 가드(POL-013).
