...
| DONE | 완료 |
| HOLD | 보류 |
| ~~BLOCKED~~ | ~~차단~~ — **CR-040: 더 이상 common_status로 시드/전이되지 않음.** 막힘은 flagged 깃발(상태 독립). enum 값은 레거시로 잔존(과거 데이터 파싱용)하나 신규 발생 없음 |
...
> 이 테이블에 없는 (from→to)는 전면 차단(화이트리스트). 막힘은 상태 전이가 아니라 flagged 깃발이므로 이 전이표와 무관(CR-040).
...
| avatar_url | VARCHAR(500) | N | | 프로필 사진 URL(CR-047, `/files/serve/*` 경로). null=이니셜 폴백 |
...
> **CR-047 프로필 사진(V19)**: `users.avatar_url` 추가(nullable). 프로필 사진 업로드는 기존 파일 업로드(`POST /files/upload`, CR-024 이미지 화이트리스트)를 재사용해 반환된 `/files/serve/*` URL을 저장. 사진이 없으면 FE에서 이름 이니셜로 폴백. 부서명은 카드 표시용으로 `departments` 조인해 노출(스키마 무변경).
...
| description | TEXT | N | | |
| status | VARCHAR(20) | Y | 'ACTIVE' | **CR-046 V18** — ACTIVE / ARCHIVED (FSM, T1-5 workspace) |
| archived_at | TIMESTAMPTZ | N | | **CR-046 V18** — 보관 시점(해제 시 null 복원). status=ARCHIVED와 정합 |
| created_by | BIGINT | Y | | → users.id |
| created_at / updated_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_workspaces_status(status)` — 목록 조회가 기본 `status='ACTIVE'`로 필터하므로(BIZ-113).
...
| require_acceptance_criteria | BOOLEAN | Y | false | **CR-049** 인수조건 미충족 시 완료(DONE) 차단 강제 여부(POL-015/BIZ-116). 기본 비강제. V21 추가 |
...
| flagged | BOOLEAN | Y | false | 막힘 깃발(Jira Flag 방식, CR-040). 상태와 독립 |
| block_reason | VARCHAR(500) | N | | 막힘 사유. flagged=true 시 필수(BIZ-005), 해제 시 null |
| **measure_unit_id** | BIGINT | N | | → measure_unit.id (측정 추상화 §8.5) |
| **target_value** | NUMERIC(18,2) | N | | 목표값 |
| **current_value** | NUMERIC(18,2) | N | | 현재값 |
| acceptance_criteria | JSONB | N | | Story 인수조건. **CR-049부터 체크 가능 구조** `[{text, checked, checkedBy, checkedAt}]`(기존 `["문장"]` 문자열 배열 → V21에서 변환). checked=사람 판정(BIZ-115). 강제 프로젝트(POL-015)에선 미충족 시 DONE 차단(BIZ-116). |
...
| **result_content** | TEXT | N | | 결과(완료 산출물) 본문. 리치텍스트 HTML(CR-024 에디터). 업무당 1개·덮어쓰기(BIZ-114, WMP-WI-017, CR-048) |
| **result_written_by** | BIGINT | N | | → users.id (결과 작성자) |
| **result_written_at** | TIMESTAMPTZ | N | | 결과 최종 작성 시각 |
| **draft** | BOOLEAN | Y | false | **CR-050** AI 업무 초안(WMP-WI-019, aimbase) 여부. true=초안(백로그에서만 구분표시, 그 외 모든 조회/집계 제외 — BIZ-117), 확정 시 false로 정식 전환. V22 추가 |
...
- `idx_wi_flagged`(flagged) — 부분 인덱스 `WHERE flagged = true` (막힘 목록 집계, CR-040)
- `idx_wi_draft`(project_id) — 부분 인덱스 `WHERE draft = true` (프로젝트별 초안 목록·전체버리기, CR-050)

> **CR-040**: 막힘을 상태 전이 → Jira Flag 방식으로 전환하며 `prev_status_id`(V1 스키마에 있었으나 이 표엔 미기재된 문서부채 컬럼) **드롭**. 막힘 시 직전 상태 복귀 로직이 폐기되어 더 이상 쓰지 않음(V14 마이그레이션에서 DROP COLUMN).
...
| **kind** | VARCHAR(20) | Y | REFERENCE | 첨부 성격(CR-051, BIZ-118): REFERENCE=참고자료(입력)·RESULT=결과물(산출물, WMP-WI-017 연동). 기존은 REFERENCE로 백필 |
| uploaded_by | BIGINT | Y | | → users.id |
| created_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_attachments_work_item`(work_item_id), `idx_attachments_kind`(work_item_id, kind) — kind별 조회
...
| action | VARCHAR(40) | Y | | STATUS_CHANGE/ASSIGN/FIELD_UPDATE/TYPE_CONVERT/LINK/FLAG_ON/FLAG_OFF/**COMPLETE_WITH_UNMET**(CR-049) |
| from_value | VARCHAR(200) | N | | |
| to_value | VARCHAR(200) | N | | |
| metadata | JSONB | N | | **CR-049** 구조화 부가정보. COMPLETE_WITH_UNMET 시 미충족 스냅샷 `{unmet:[text...], met:N, total:M}`(BIZ-116 책임 소지). V21 추가 |
| created_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_activity_logs_work_item`(work_item_id, created_at)

> **COMPLETE_WITH_UNMET(CR-049)**: 비강제 프로젝트(require_acceptance_criteria=false)에서 인수조건 미충족 항목이 있는 채 DONE 진입 시 STATUS_CHANGE와 별도로 1건 기록. actor_id·created_at으로 "누가·언제" 확보, metadata JSONB로 "어떤 항목이 미충족이었나" 스냅샷 보존(완료 후 뒤늦게 체크해도 완료 당시 상태 불변).
...
## 건강 지표 신규 테이블 (설계만 — 2·3차 구현) — CR-043

> **프로젝트 건강 지표 체계(CR-043)의 흐름·예측·일정 축을 위한 스냅샷/동결 테이블 3종.** burndown_snapshots 패턴(스냅샷 + UNIQUE 멱등 인덱스 + 스케줄러/이벤트 소비)을 재사용한다.
> **1차 구현은 신규 테이블 0** — Age(멈춘 일)·재작업률·현장검증·담당자 과부하·운영 적용률은 기존 `work_items`(flagged/common_status/completed_at/assignee) + `activity_logs`(STATUS_CHANGE) + `field_verifications` 집계로 산출한다. 아래 3종은 2·3차에 신설한다.

### flow_snapshots (CFD/일별 상태 스냅샷) — WMP-HOME-007, CR-043 [2차, V15]
| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BIGINT | PK | auto | |
| project_id | BIGINT | Y | | → projects.id |
| snapshot_date | DATE | Y | | 스냅샷 일자(하루 1회 롤업) |
| common_status | VARCHAR(20) | Y | | 집계 상태군(TODO/IN_PROGRESS/DONE 등, CommonStatusGroup) |
| item_count | INT | Y | 0 | 해당 일자·상태의 work item 수(누적 흐름도 밴드 높이) |
| created_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `uniq_flow_project_date_status`(project_id, snapshot_date, common_status, UNIQUE — 멱등), `idx_flow_project_date`(project_id, snapshot_date)
> **출처(설계골격 §4)**: CFD(누적 흐름도, WMP-HOME-007)용 프로젝트×상태×날짜 카운트. burndown_snapshots처럼 일별 스케줄러가 ACTIVE 프로젝트의 상태별 항목 수를 롤업 UPSERT(멱등). 밴드 평행·Done 라인 비평평 여부로 흐름 건강 판단(F). work_item_id를 두지 않고 상태별 롤업만 저장.

### sprint_commitments (Say-Do 커밋 동결값) — WMP-HOME-008, CR-043 [2차, V16]
| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BIGINT | PK | auto | |
| sprint_id | BIGINT | Y | | → sprints.id |
| committed_item_count | INT | Y | 0 | 스프린트 시작 시점 커밋 항목 수(동결) |
| committed_points | INT | Y | 0 | 스프린트 시작 시점 커밋 스토리포인트 합(동결) |
| committed_item_ids | JSONB | Y | '[]' | 시작 시점 커밋된 work item id 배열(스코프 변경률 산출용) |
| frozen_at | TIMESTAMPTZ | Y | now() | 동결 시각(SprintStarted 시점) |

**인덱스**: `uniq_commitment_sprint`(sprint_id, UNIQUE — 스프린트당 1행, 멱등)
> **출처(설계골격 §4)**: Say-Do Ratio(약속÷완료, WMP-HOME-008)와 스코프 변경률(B축)의 분모용 동결값. `SprintStarted` 이벤트 소비 시 1회 스냅샷(멱등). Say-Do = 완료 포인트 ÷ committed_points, 스코프 변경률 = 시작 후 committed_item_ids 밖에서 추가된 항목 비율. 건강 신호 80~110%(F).

### project_baselines (EVM baseline — 계획선 PV) — WMP-HOME-013, CR-043 [3차, V17]
| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| id | BIGINT | PK | auto | |
| project_id | BIGINT | Y | | → projects.id |
| baseline_date | DATE | Y | | 기준선 확정 일자 |
| planned_points | INT | Y | 0 | 계획 총 스토리포인트(BAC — Budget At Completion) |
| planned_end_date | DATE | Y | | 계획 완료일(PV 곡선 종점) |
| is_active | BOOLEAN | Y | true | 현재 유효 기준선(재설정 시 이전 행 false) |
| created_at | TIMESTAMPTZ | Y | now() | |

**인덱스**: `idx_baseline_project_active`(project_id, is_active), `uniq_baseline_project_date`(project_id, baseline_date, UNIQUE — 멱등)
> **출처(설계골격 §4)**: EVM(획득가치, WMP-HOME-013)의 PV(Planned Value) 곡선을 위한 baseline — 계획 완료일·계획 포인트를 동결. SPI = EV÷PV, CPI = EV÷AC (EV·AC는 실적 집계). 기준선 재설정 시 이전 행 is_active=false로 이력 보존. 3차(신설 개념).
...
## Flyway 마이그레이션 V15~V17 — 건강 지표 스냅샷/동결 (CR-043, 2·3차)
- **V15 `flow_snapshots`**(CFD, WMP-HOME-007) · **V16 `sprint_commitments`**(Say-Do 동결, WMP-HOME-008) · **V17 `project_baselines`**(EVM baseline, WMP-HOME-013) — 위 "건강 지표 신규 테이블" 절 정의. 모두 스키마 추가만, 기존 테이블 무변경.
- **1차 구현은 신규 테이블 0** — 멈춘 일 Age·재작업률·현장검증·담당자 과부하는 `work_items.flagged` + `activity_logs` 집계로 산출한다. V15~V17은 2·3차에 착수.

## Flyway 마이그레이션 V18 — 워크스페이스 보관 상태 (CR-046)
- `ALTER TABLE workspaces ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE'` + `ADD COLUMN archived_at TIMESTAMPTZ NULL`.
- 기존 행 전부 `status='ACTIVE'`로 백필(DEFAULT로 자동). `archived_at`은 null.
- `CREATE INDEX idx_workspaces_status ON workspaces(status)` — 목록 조회가 기본 ACTIVE 필터(BIZ-113).
- 스키마 추가만, 하위 테이블(projects/workspace_members/chat 등) 무변경. 보관은 동결이라 하위 컬럼 변화 없음.

## Flyway 마이그레이션 V21 — 인수조건 완료 강제 + 체크 가능 구조 (CR-049)
- `ALTER TABLE projects ADD COLUMN require_acceptance_criteria BOOLEAN NOT NULL DEFAULT false` — 프로젝트별 강제 토글(POL-015). 기존 행 전부 false(비강제) 백필.
- `ALTER TABLE activity_logs ADD COLUMN metadata JSONB NULL` — COMPLETE_WITH_UNMET 스냅샷 등 구조화 부가정보.
- **acceptance_criteria 구조 변환**(컬럼 타입 무변경, 값만 재구성): 기존 `["문장1","문장2"]`(문자열 배열) → `[{"text":"문장1","checked":false,"checkedBy":null,"checkedAt":null}, ...]`. UPDATE 문에서 `jsonb_array_elements_text`로 각 원소를 객체로 래핑. NULL·빈 배열·이미 객체 구조(재실행 방어)는 건너뜀. 배포 시점 운영 데이터에 인수조건 보유 항목이 있으면 이 변환으로 호환.
- checklist는 무변경(String 패스스루 유지, 이번 CR 대상 아님 — BIZ-115).
- 스키마: projects 1컬럼 + activity_logs 1컬럼 추가, work_items 컬럼 타입 변경 없음(JSONB 유지).
