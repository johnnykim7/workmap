...
## DONE 전이 인수조건 가드 (CR-049, BIZ-115/116)

> 완료(DONE 계열) 진입 시, 프로젝트 정책(POL-015)이 강제이면 인수조건 미충족을 추가로 검증한다. 승인 게이트(BIZ-110)와 **독립적인 별개 가드**다.

- **적용 대상 상태**: `is_done=true`인 모든 상태(개발형 DONE / 운영형 DONE / 범용 DONE / 현장검증형 OPS_APPLIED). 워크플로 종류 무관, `is_done` 플래그로 판별.
- **가드 조건**: 대상 항목이 속한 프로젝트의 `require_acceptance_criteria=true`이고, 항목의 `acceptance_criteria`에 **1개 이상 존재하며 그중 미충족(checked=false)이 하나라도 있으면** DONE 전이를 거부한다(`ACCEPTANCE_CRITERIA_UNMET`, WMP-7850).
  - 인수조건이 아예 없는 항목(빈 배열/null)은 강제 대상 아님 — 통과(운영형·인수조건 미사용 업무 보호).
  - `require_acceptance_criteria=false`(기본, 비강제)면 이 가드는 검증하지 않고 통과. 단 미충족 상태로 완료되면 부수효과로 활동 로그에 미충족 스냅샷 기록(BIZ-116, 아래).
- **검증 순서**: work_item FSM 화이트리스트 통과 → (게이트 상태면) 승인 결합 가드 → **인수조건 가드** → 부수효과(completed_at 세팅·이벤트·활동로그). 즉 화이트리스트·승인을 이미 통과한 전이만 이 가드가 최종 검사한다.
- **비강제 시 부수효과(책임 소지)**: 비강제 프로젝트에서 미충족 항목이 있는 채 DONE 진입하면, 상태 변경 활동 로그(STATUS_CHANGE)와 별개로 미충족 스냅샷을 활동 로그에 기록한다(action=`COMPLETE_WITH_UNMET`, metadata=미충족 항목 텍스트 목록·충족/전체 수). 실행자(actor)·시점(created_at)은 활동 로그가 기본 제공.
- **재오픈**: DONE→비완료 역전이는 이 가드와 무관(completed_at=null 리셋만, 기존 규칙 유지).
- **체크리스트 제외**: checklist는 이 가드 대상이 아니다(BIZ-115) — 강제/차단에 관여하지 않음.
