...
- **진입 조건**: 스프린트 만들기
- **허용 다음 상태**: ACTIVE
- **비고**: `[스프린트 시작]` 버튼 — **앞 스프린트가 ACTIVE면 차단(회색)**. 동시 ACTIVE 1개/프로젝트 제약.
...
- **허용 다음 상태**: COMPLETED
- **비고**: `[스프린트 완료]` 버튼. 프로젝트당 1개만 존재 가능.
- **관련 기능 ID**: WMP-AGL-003, WMP-AGL-005(보드)

### COMPLETED | 완료
- **설명**: 종료된 스프린트.
- **진입 조건**: 스프린트 완료
- **허용 다음 상태**: (최종 상태)
- **비고**: **완료 시 미완료 항목 → 다음 스프린트 또는 백로그로 이월(선택)**. 완료 후 ACTIVE로 되돌릴 수 없음.
